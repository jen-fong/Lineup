...
function checkIfParkIsOpen (opHours) {
  ...
  const dbParkTimeClosed = moment(parkClose).utcOffset('-05:00')
  const dbParkTimeOpen = moment(parkOpen).utcOffset('-05:00')
  const dbSpecialTimeOpen = specialHours && moment(specialHoursOpen).utcOffset('-05:00')
  const dbSpecialTimeClose = specialHours && moment(specialHoursClose).utcOffset('-05:00')

  const isOpen = now.isBetween(dbParkTimeOpen, dbParkTimeClosed, 'hour', [])
  const isSpecialHours = now.isBetween(dbSpecialTimeOpen, dbSpecialTimeClose, [])
